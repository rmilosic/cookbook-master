...
export async function buildPdf(ocrDict: object): Promise<string> {


    return new Promise((resolve, reject) => {

        var pages: Array<number> = [0];

        const blocks: Array<object> = ocrDict["Blocks"]

        console.log("filtering")
        const lines: Array<object> = blocks.filter(block => {
